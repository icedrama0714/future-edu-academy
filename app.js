...
function cleanupDuplicatePaymentRecords() {
  ...
  students = students.map((student) => {
    ...
    const isSeoYoonCollision = canonicalStudentName(student.studentName) === "이서윤"
...
function distributionRecordedPaymentTexts() {
  const latestByStudent = new Map();
  [...SEEDED_PAYMENT_RECORDS, ...standalonePaymentRecords].forEach((record) => {
    const name = canonicalStudentName(record.studentName);
    if (!name) return;
    const date = String(record.registered || record.paymentDate || record.paymentDueDate || "");
    const previous = latestByStudent.get(name);
    if (!previous || date > previous.date) {
      latestByStudent.set(name, {
        date,
        text: `${record.paymentName || ""} ${record.memo || ""}`,
      });
    }
  });
  return new Map(Array.from(latestByStudent, ([name, value]) => [name, value.text]));
}

function distributionTeacherGroups(student, recordedPaymentTexts = new Map()) {
  ...
  const recordedPaymentText = recordedPaymentTexts.get(canonicalStudentName(student?.studentName)) || "";
  const paymentText = `${student?.paymentName || ""} ${recordedPaymentText}`.replace(/\s+/g, " ");
...
function classDistributionData(month, teacherFilter = "") {
  ...
  const recordedPaymentTexts = teacherFilter ? distributionRecordedPaymentTexts() : new Map();
  const roster = distributionRosterStudents().filter((student) => (
    !teacherFilter || distributionTeacherGroups(student, recordedPaymentTexts).includes(teacherFilter)
  ));
...
function buildLearningReportDraft(student, start, end, records, reportType = "monthly") {
  ...
  const cleanLatestNote = latestNote ? latestNote.trim() : "";
  const punctuatedLatestNote = cleanLatestNote && !/[.!?]$/.test(cleanLatestNote)
    ? `${cleanLatestNote}.`
    : cleanLatestNote;
  const noteSentence = punctuatedLatestNote ? ` 최근 수업 기록입니다. ${punctuatedLatestNote}` : "";
...
async function requestAiLearningReport(context) {
  if (window.location.protocol === "file:") return "";
  try {
    const response = await fetch("/api/generate-learning-report", {
      method: "POST",
    ...
    return String(result.report || "").trim();
  } catch (error) {
...
async function generateLearningReport() {
  ...
  const button = $("generateLearningReportBtn");
...
async function previewLearningReport() {
  if (!$("learningReportText")?.value.trim()) await generateLearningReport();
  renderLearningReportPreview();
}

async function printLearningReport() {
  if (!$("learningReportText")?.value.trim()) await generateLearningReport();
  const text = $("learningReportText")?.value.trim();
  if (!text) {
...
function bindEvents() {
  ...
  $("generateLearningReportBtn")?.addEventListener("click", async () => {
